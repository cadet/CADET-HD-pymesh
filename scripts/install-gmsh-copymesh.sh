...
# COPIED from install-gmsh-and-deps.sh 
#
# In those cases when nix (and nix-user-chroot) is not available on a system
# where gmsh/pymesh needs to be deployed, use this script as a guide.
#
# On JURECA: You might need to run:
#     module load OpenGL/<version>
# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
...
install_gmsh_from_git()
{
    ...
    clone_or_pull "$GMSH_GIT_LINK" gmsh
    [[ ! -f custom_mesh_copy.patch ]] && echo "No patch file in directory" && exit -1
    cd gmsh
    git checkout 2ac03e26721ff5ffe20759ef4ad474da6cbf4b44
    cp ../custom_mesh_copy.patch .
    patch -p1 < custom_mesh_copy.patch
    mkdir -p build
    cd build
